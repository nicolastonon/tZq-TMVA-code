...
theMVAtool::theMVAtool(std::vector<TString > thevarlist, std::vector<TString > thesamplelist, std::vector<TString > thesystlist, std::vector<TString > thechanlist, vector<int> v_color, std::vector<TString > set_v_cut_name, std::vector<TString > set_v_cut_def, std::vector<bool > set_v_cut_IsUsedForBDT, vector<TString> v_add_vars, int nofbin_templates, bool in_isttZ, bool in_isWZ, TString extension_format, bool set_combine_naming_convention, TString set_dir_ntuples, TString set_t_name, vector<TString> thevar_list_BDTcut, bool set_draw_preliminary_label)
{
	...
	for(int i=0; i<thevar_list_BDTcut.size(); i++) //CHANGED
	{
		var_list_BDTcut.push_back(thevar_list_BDTcut[i]);
		BDTcut_floats.push_back(-999);

		// for(int ivar=0; ivar<v_add_vars.size(); ivar++)
		// {
		// 	if(v_add_vars[ivar] == thevar_list_BDTcut[i])
		// 	{
		// 		cout<<FGRN("** Constructor")<<" : erased variable "<<v_add_vars[ivar]<<" from vector v_add_vars (possible conflict) !"<<endl;
		// 		v_add_vars.erase(v_add_vars.begin() + ivar);
		// 	}
	...
	for(int ivar=0; ivar<v_cut_name.size(); ivar++)
	{
		...
		if(v_cut_name[ivar]=="ContainsBadJet" || v_cut_name[ivar]=="fourthLep10") {continue;} //Default, no need to appear in filename
	...
	//suffix needed only in case of 2D scan (during rescaling)
	this->filename_suffix_noJet_noMET_nomTW = "";
	tmp = "";
	for(int ivar=0; ivar<v_cut_name.size(); ivar++)
	{
		if(v_cut_name[ivar]=="NJets" || v_cut_name[ivar]=="NBJets" || v_cut_name[ivar]=="METpt" || v_cut_name[ivar]=="mTW") {continue;}
		...
		if(v_cut_def[ivar] != "")
		{
			...
			this->filename_suffix_noJet_noMET_nomTW+= tmp;
		}
	}
	...
	draw_preliminary_label = set_draw_preliminary_label;
 ...
 */
void theMVAtool::Train_Test_Evaluate(TString channel, TString template_name, bool write_ranking_info)
{
	...
	TString output_file_name = "outputs/" + template_name;
	...
	TMVA::Factory* factory = new TMVA::Factory(template_name.Data(), output_file, "!V:!Silent:Color:DrawProgressBar:AnalysisType=Classification" );
	...
	for(int isample=0; isample<sample_list.size(); isample++)
    {
		...
		else if(template_name == "BDTfakeSR")
		{
			if(!sample_list[isample].Contains("Fakes") && !sample_list[isample].Contains("tZq") ) {continue;}
		}
		else
		{
		...
		// if(!sample_list[isample].Contains("WZ") && !sample_list[isample].Contains("ttZ") && !sample_list[isample].Contains("ZZ") && !sample_list[isample].Contains("tZq") && !sample_list[isample].Contains("Fakes") ) {continue;}
    ...
    //-- Choose between absolute/relative weights for training
		if(template_name == "BDTfake")
		{
			...
			if(sample_list[isample].Contains("Fakes")) {factory->AddSignalTree ( tree, signalWeight ); factory->SetSignalWeightExpression( "fabs(Weight)" );}
...
//--------------------------------
	if(mycuts != mycutb) {cout<<__LINE__<<FRED("PROBLEM : cuts are different for signal and background ! If this is normal, fix code -- Abort")<<endl; return;}
...
//--------------------------------------
	...
	if(!isWZ && !isttZ) ranking_file_path = "outputs/Rankings/BDT/RANKING_" + template_name + "_" + channel + ".txt";
	else if(!isWZ && isttZ) ranking_file_path = "outputs/Rankings/BDTttZ/RANKING_" + template_name + "_" + channel + ".txt";
	else if(isWZ) ranking_file_path = "outputs/Rankings/BDTfake/RANKING_" + template_name + "_" + channel + ".txt";
	...
	if(write_ranking_info)
	{
		...
		Extract_Ranking_Info(ranking_file_path, template_name, channel); //Extract only ranking info from TMVA output
	}
 ...
 */
double theMVAtool::Compute_Fake_SF(TFile* f, TString channel, bool fit_FakeElandMu_together, TString FakeName, TString template_name)
{
	bool fit_WZ_separately = false; //CHANGED
	...
	if(!fit_FakeElandMu_together && channel=="uuu" && FakeName=="FakeEl") {return 0;}
	if(!fit_FakeElandMu_together && channel=="eee" && FakeName=="FakeMu") {return 0;}
	...
	for(int isample=0; isample<sample_list.size(); isample++)
	{
		...
		if(fit_WZ_separately && sample_list[isample].Contains("WZ")) {continue;}

		// TString name_tmp = "mTW_"+channel+"__"+sample_list[isample];
		TString name_tmp = template_name + "_"+channel+"__"+sample_list[isample];
	...
	f->cd();
	TH1F * hdata=0, *hsum=0, *hfake=0, *h_tmp=0, *h_fakeEl=0, *h_fakeMu=0, *h_WZ=0;
	...
	if(combine_naming_convention) histo_name = template_name + "_" + channel + "__data_obs";
	// else histo_name = "mTW_" + channel + "__DATA";
	else histo_name = template_name + "_" + channel + "__DATA";
	...
	histo_name = template_name + "_" + channel + "__FakesElectron";
	// histo_name = "mTW_" + channel + "__FakesElectron";
	if ( !f->GetListOfKeys()->Contains( histo_name.Data() ) )  {cout<<histo_name.Data()<<" not found !"<<endl; return 0;}
	h_tmp = (TH1F*)f->Get( histo_name);
	if(fit_FakeElandMu_together) {hfake = (TH1F*) h_tmp->Clone();}
	else {h_fakeEl = (TH1F*) h_tmp->Clone();}

	histo_name = template_name + "_" + channel + "__FakesMuon";
	// histo_name = "mTW_" + channel + "__FakesMuon";
	if ( !f->GetListOfKeys()->Contains( histo_name.Data() ) )  {cout<<histo_name.Data()<<" not found !"<<endl; return 0;}
	h_tmp = (TH1F*)f->Get( histo_name);
	if(fit_FakeElandMu_together) {hfake->Add(h_tmp);}
	else {h_fakeMu = (TH1F*) h_tmp->Clone();}

	//MC
	for (int i = 0; i<v_otherBackgrounds.size(); i++)
	{
		histo_name = template_name + "_" + channel + "__" + v_otherBackgrounds[i];
	...
	if(fit_WZ_separately)
	{
		h_WZ = 0;

		for (int isample = 0; isample<sample_list.size(); isample++)
		{
			if(!sample_list[isample].Contains("WZ")) continue;

			histo_name = template_name + "_" + channel + "__" + sample_list[isample];
			// histo_name = "mTW_" + channel + "__" + v_otherBackgrounds[i];
			if ( !f->GetListOfKeys()->Contains( histo_name.Data() ) ) {cout<<histo_name.Data()<<" not found !"<<endl; return 0;}
			h_tmp = (TH1F*)f->Get( histo_name);
	...
	if(!hdata || !hsum || (fit_FakeElandMu_together && !hfake) || (!fit_FakeElandMu_together && (!h_fakeMu || !h_fakeEl)) ) {cout<<"Warning : null histogram --> CAN NOT RESCALE FAKE HISTOGRAMS !"<<endl; return 0;}


	TObjArray *mc = 0;
	if(fit_FakeElandMu_together || channel=="uuu" || channel=="eee") mc = (TObjArray*) new TObjArray(2); //Create array of MC samples -- differentiate fakes from rest
	else mc = (TObjArray*) new TObjArray(3);

	if(fit_FakeElandMu_together)
	{
		mc->Add(hfake); //Param 0
		mc->Add(hsum); //Param 1
		if(fit_WZ_separately) mc->Add(h_WZ); //Param 2
	}
	else
	{
		...
		if(fit_WZ_separately) mc->Add(h_WZ); //Param 2 or 3
	}

	TFractionFitter* fit = new TFractionFitter(hdata, mc); //'Q' for quiet

	//Constrain backgrounds which are not fake (NB : because we're only interested in fitting the fakes to the data here!)
	double fracsum = hsum->Integral()/hdata->Integral() ;
	double fracwz = 0;
	...
	if(status != 0) {cout<<BOLD(FRED("ERROR ! Problem during fit !"))<<endl; return 0;} //check status
	...
	double fraction_WZ=0, error_WZ=0;

	cout<<"--- CHANNEL "<<channel<<" ---"<<endl;
	cout<<endl<<"(Frac. other bkgs before fit = "<<fracsum<<")"<<endl;

	if(fit_FakeElandMu_together)
	{
		fit->GetResult(0, fraction_fake, error_fake);
		fit->GetResult(1, fraction_other, error_other);
		if(fit_WZ_separately) fit->GetResult(2, fraction_WZ, error_WZ);
		cout<<FGRN("Fraction other = "<<fraction_other<<", error = "<<error_other<<" ")<<endl;
		if(fit_WZ_separately) cout<<FGRN("Fraction WZ = "<<fraction_WZ<<", error = "<<error_WZ<<" ")<<endl;
	...
	else
	{
		...
		if(channel != "uuu" && channel != "eee")
		{
			...
			if(fit_WZ_separately) fit->GetResult(3, fraction_WZ, error_WZ);
	...
	cout<<"Fit proba. = "<<fit->GetProb()<<endl;
	...
	if(fit_FakeElandMu_together) {SF = fraction_fake * (hdata->Integral() / hfake->Integral() );} //Return 'postfit fakes fraction' over 'prefit fakes fraction' ==> SF
	else
	{
	...
	if(fit_FakeElandMu_together) delete hfake;
	else
	{
...
void theMVAtool::Rescale_Fake_Histograms(TString file_to_rescale_name, TString template_name)
{
	...
	if(template_name != "mTW" && template_name != "BDTfake") {cout<<"Wrong template name !"<<endl; return;}

	bool fit_FakeElandMu_together = true; //fit FakeMu and FakeEl templates altogether, for each channel
	...
	file_mTW_templates_unscaled_PATH = "outputs/Reader_" + template_name + this->filename_suffix_noJet_noMET_nomTW + "_NJetsMin0_NBJetsEq0_unScaled.root"; //mTW unrescaled Template file  => Used to compute the Fakes SFs //Need to use this suffix in case we're doing a 2D scan on MET/mTW, don't want them in the mTW filename

	if(!Check_File_Existence(file_mTW_templates_unscaled_PATH))
	{
		cout<<file_mTW_templates_unscaled_PATH<<" doesn't exist !"<<endl;
		// file_mTW_templates_unscaled_PATH = "outputs/Reader_mTW_NJetsMin0_NBJetsEq0_unScaled.root"; //default
		file_mTW_templates_unscaled_PATH = "outputs/Reader_" + template_name + "_NJetsMin0_NBJetsEq0_unScaled.root"; //default
	...
	//--- To use other template file than mTW distribution to do the rescaling (m3l, METpt, ...)
	// file_mTW_templates_unscaled_PATH = "outputs/Reader_METpt" + this->filename_suffix_noJet_noMET_nomTW + "_NJetsMin0_NBJetsEq0_unScaled.root";
	// if(!Check_File_Existence(file_mTW_templates_unscaled_PATH)) return;
	...
	if(fit_FakeElandMu_together)
	{
		SF_uuu = Compute_Fake_SF(file_mTW_templates_unscaled, "uuu", fit_FakeElandMu_together, "", template_name);
		SF_uue = Compute_Fake_SF(file_mTW_templates_unscaled, "uue", fit_FakeElandMu_together, "", template_name);
		SF_eeu = Compute_Fake_SF(file_mTW_templates_unscaled, "eeu", fit_FakeElandMu_together, "", template_name);
		SF_eee = Compute_Fake_SF(file_mTW_templates_unscaled, "eee", fit_FakeElandMu_together, "", template_name);
	}
	else
	{
		SF_uuu_FakeMu = Compute_Fake_SF(file_mTW_templates_unscaled, "uuu", fit_FakeElandMu_together, "FakeMu", template_name);
		SF_uue_FakeMu = Compute_Fake_SF(file_mTW_templates_unscaled, "uue", fit_FakeElandMu_together, "FakeMu", template_name);
		SF_eeu_FakeMu = Compute_Fake_SF(file_mTW_templates_unscaled, "eeu", fit_FakeElandMu_together, "FakeMu", template_name);
		SF_eee_FakeMu = Compute_Fake_SF(file_mTW_templates_unscaled, "eee", fit_FakeElandMu_together, "FakeMu", template_name);
		SF_uuu_FakeEl = Compute_Fake_SF(file_mTW_templates_unscaled, "uuu", fit_FakeElandMu_together, "FakeEl", template_name);
		SF_uue_FakeEl = Compute_Fake_SF(file_mTW_templates_unscaled, "uue", fit_FakeElandMu_together, "FakeEl", template_name);
		SF_eeu_FakeEl = Compute_Fake_SF(file_mTW_templates_unscaled, "eeu", fit_FakeElandMu_together, "FakeEl", template_name);
		SF_eee_FakeEl = Compute_Fake_SF(file_mTW_templates_unscaled, "eee", fit_FakeElandMu_together, "FakeEl", template_name);
	}
	...
	total_var_list.push_back("mTWandBDT"); //CHANGED
	total_var_list.push_back("BDT2l"); //CHANGED
	total_var_list.push_back("BDTfakeSR"); //CHANGED
...
void theMVAtool::Vector_isEventPassingBDTfakeSRCut(vector<bool> &v, TString sample, TString systname, double cut_value, bool keep_high_BDT_events)
{
	...
	TMVA::Reader *reader_cut = new TMVA::Reader("Silent"); //CHANGED -- need other reader only to cut on BDTfakeSR (different variables)
	...
	{
		reader_cut->AddVariable(var_list_BDTcut[i].Data(), &BDTcut_floats[i]);
	...
	for(int i=0; i<v_cut_name.size(); i++)
	{
		...
		{
			if(print_spectator_vars) cout<<v_cut_name[i]<<endl;
			reader_cut->AddSpectator(v_cut_name[i].Data(), &v_cut_float[i]);
		}
	}
	for(int i=0; i<v_add_var_names.size(); i++)
	{
	...
	{
		TString dir    = "weights/";
 		TString MVA_method_name = "BDTfakeSR_" + channel_list[ichan] + this->filename_suffix_noJet + "_NJetsMin1Max4_NBJetsEq1" + TString(" method");
		TString weightfile = dir + "BDTfakeSR_" + channel_list[ichan] + this->filename_suffix_noJet + "_NJetsMin1Max4_NBJetsEq1" + TString(".weights.xml");

		reader_cut->BookMVA(MVA_method_name, weightfile);
	}
	...
	{
		tree->SetBranchAddress(var_list_BDTcut[ivar], &BDTcut_floats[ivar]);
	}
	...
	for(int ientry=0; ientry<nentries; ientry++)
	{
		...
		else {cout<<FRED("ERROR channel!")<<endl; return;}

		TString MVA_method_name = "BDTfakeSR_" + chan_name + this->filename_suffix + TString(" method");

		// if(ientry==0)	cout<<reader_cut->EvaluateMVA(MVA_method_name)<<endl;

		if(keep_high_BDT_events && reader_cut->EvaluateMVA(MVA_method_name) < cut_value) {v.push_back(false);}
		else if(!keep_high_BDT_events && reader_cut->EvaluateMVA(MVA_method_name) >= cut_value) {v.push_back(false);}
		else {v.push_back(true); nevents_passingCut++;}
	}

	if(v.size() != nentries) {cout<<"Wrong number of entries in BDT cut vector ! Check it !"<<endl; return;}
 ...
 */
int theMVAtool::Read(TString template_name, bool fakes_from_data, bool real_data, bool cut_on_BDT, bool keep_high_BDT_events, double BDT_cut_value, bool cut_on_BDTfakeSR)
{
	cout<<endl<<BOLD(FYEL("##################################"))<<endl;
	...
	if(template_name.Contains("BDT") || cut_on_BDT) //Need to book method if want BDT template or if cut on BDT
	{
		...
		for(int ichan=0; ichan<channel_list.size(); ichan++) //Book the method for each channel (separate BDTs)
		{
			...
			if(template_name == "mTWandBDT" || (cut_on_BDT && !template_name.Contains("BDT")) ) {template_name_MVA = "BDT";} //Need to book BDT-tZq for these
	...
	for(int isyst=0; isyst<syst_list.size(); isyst++)
	{
		...
		for(int isample=0; isample<sample_list.size(); isample++)
		{
			...
			else if(syst_list[isyst].Contains("JESFWD2j")) tree = (TTree*) file_input->Get("JESFWD");
...
//WARNING : the last SetBranchAddress overrides the previous ones !! Be careful not to associate branches twice !
			...
			// tree->SetBranchAddress("METpt", &mTW); //If want to replace mTW distribution by something else (e.g. m3l or MET for prefit), could simply replace it here without changing anything else in code !
...
// --- START EVENT LOOP ---
			...
			for(int ievt=0; ievt<n_entries; ievt++)
			{
				...
				if(cut_on_BDTfakeSR && template_name == "BDT" && !v_isEventPassingBDTfakeSRCut[ievt]) {continue;}
		        ...
		        for(unsigned int ichan=0; ichan < channel_list.size(); ichan++)
				{
					...
					//cout << "i_channel " << i_channel << endl;
		          	...
		          	else if(i_channel == 1 && channel_list[ichan] == "uue")  isChannelToKeep = true;
		          	else if(i_channel == 2 && channel_list[ichan] == "eeu")  isChannelToKeep = true;
		          	else if(i_channel == 3 && channel_list[ichan] == "eee")  isChannelToKeep = true;
				}
...
//--- CREATE OUTPUT HISTOGRAMS (TEMPLATES) ----
				...
				if(template_name.Contains("BDT") )
				{
					if(i_channel == 0)
					{
						...
						{
							mva_value = reader->EvaluateMVA( "BDT_uuu"+this->filename_suffix+ " method");
					...
					else if(i_channel == 1)
					{
						...
						{
							mva_value = reader->EvaluateMVA( "BDT_uue"+this->filename_suffix+ " method");
							mva_value = mva_value*50 + 50 + 50;
					...
					else if(i_channel == 2)
					{
						...
						{
							mva_value = reader->EvaluateMVA( "BDT_eeu"+this->filename_suffix+ " method");
							mva_value = mva_value*50 + 50 + 50;
					...
					else if(i_channel == 3)
					{
						...
						{
							mva_value = reader->EvaluateMVA( "BDT_eee"+this->filename_suffix+ " method");
							mva_value = mva_value*50 + 50 + 50;
...
//---------
			...
			if(syst_list[isyst].Contains("Zpt") || syst_list[isyst].Contains("Fake"))
			{
				...
				else if(sample_list[isample] == "FakesMuon")
				{
				...
				else cout<<"Error -- check fakes sample name !"<<endl;
			}
	...
	delete file_output;
	delete reader; //Free memory
	std::cout << "==> Reader() is done!" << std::endl << std::endl;

	if(fakes_from_data)
	{
		if(template_name=="BDTfake") Rescale_Fake_Histograms(output_file_name, template_name);
		else Rescale_Fake_Histograms(output_file_name, "mTW");
	}
...
void theMVAtool::Create_Control_Histograms(bool fakes_from_data, bool use_pseudodata_CR_plots, bool cut_on_BDT)
{
		...
		for(int ivar=0; ivar<total_var_list.size(); ivar++)
		{
			...
			for(int isample = 0; isample < sample_list.size(); isample++)
			{
				...
				for(int isyst=0; isyst<syst_list.size(); isyst++)
				{
					...
					else if(total_var_list[ivar] == "ptQ")							{h_tmp = new TH1F( "","", control_binning, 30, 200 );} //CHANGED
					...
					else if(total_var_list[ivar] == "fourthLep10")		{h_tmp = new TH1F( "","", binning, 0, 3 );}
					else if(total_var_list[ivar] == "fourthLep15")		{h_tmp = new TH1F( "","", binning, 0, 3 );}
	...
	if(fakes_from_data) Rescale_Fake_Histograms(output_file_name, "mTW");
	// if(fakes_from_data) Rescale_Fake_Histograms(output_file_name, "BDTfake"); //Rather uyse
...
int theMVAtool::Draw_Control_Plots(TString channel, bool fakes_from_data, bool allchannels, bool postfit, bool cut_on_BDT)
{
	...
	{
		//CombineHarvester doesn't rescale automatically signal to Signal strength (in PostfitInputVars files)! Need to do it manually, extracting its value from mlfit.root !
	...
	std::vector<TString> stringv1_list;
	stringv1_list.resize( total_var_list.size() );
...
//Retrieve histograms, stack, compare, plot
	...
	//   TLegend* qw = new TLegend(.2,.4,0.359,0.739); //left legend better ??
		...
		if(!draw_preliminary_label) {c1->SetTopMargin(0.1); c1->SetBottomMargin(0.25);} //Leave space for legend --only if there isn't 'Prelim' label
		else c1->SetBottomMargin(0.3); //Leave space for ratio plot
		...
		gr_error = new TGraphAsymmErrors(nofbins,x,y,exl,exh,eyl,eyh);
		// gr_error->SetFillStyle(3013);
		gr_error->SetFillStyle(3002);
		gr_error->SetFillColor(1);
		gr_error->Draw("e2 same"); //Superimposes the systematics uncertainties on stack
...
*/
		...
		double minCsvPlot = 0.05; //Modify here binning of btagDiscri
		int nbcsv = 21;
		...
		else if(total_var_list[ivar] == "AddLepAsym")   {  stringv1_list[ivar] = "Events/0.25" ;   stringv_list[ivar] = " q_{#font[12]{l}}|#eta(#font[12]{l})|"; }
		else if(total_var_list[ivar] == "mtop")                 {       stringv_list[ivar] = "M_{top} [GeV]"; }
		else if(total_var_list[ivar] == "btagDiscri")   {   double binsiCSV= (1.-minCsvPlot)/nbcsv; stringv1_list[ivar] = "Events/0.05" ;     stringv_list[ivar] = "CSVv2 discriminant"; }
		else if(total_var_list[ivar] == "etaQ")         { stringv1_list[ivar] = "Events/0.45" ;      stringv_list[ivar] = "#eta(#font[12]{j'})"; }
		...
		else if(total_var_list[ivar] == "LeadJetPT")            {  stringv1_list[ivar] = "Events/11 GeV";     stringv_list[ivar] = "p_{T}(#font[12]{j'}) [GeV]"; }
		...
		else if(total_var_list[ivar] == "MEMvar_1")             { stringv1_list[ivar] = "Events/1.5" ;    stringv_list[ivar] = "MEM log(w_{tZq})"; }
		...
		if(!draw_preliminary_label){ pad_ratio->SetTopMargin(0.75);}
		else {pad_ratio->SetTopMargin(0.7);}
		...
		TGraphAsymmErrors *thegraph_tmp = (TGraphAsymmErrors*) gr_error->Clone();
		...
		TGraphAsymmErrors *gr_ratio_error = new TGraphAsymmErrors(thegraph_tmp->GetN(), theX , theY ,  theErrorX_l, theErrorX_h, theErrorY_l, theErrorY_h);
		// gr_ratio_error->SetFillStyle(3013);
		gr_ratio_error->SetFillStyle(3002);
		gr_ratio_error->SetFillColor(1);
		gr_ratio_error->Draw("e2 same"); //Syst. error for Data/MC ; drawn on canvas2 (Data/MC ratio)
		...
		if(stack!= 0)
		{
			...
			stack->GetYaxis()->SetTitle(stringv1_list[ivar].Data());
		}
		...
		// delete pad_ratio;
		delete qw; delete stack; delete gr_error;
...
{
	cout<<endl<<BOLD(FYEL("##################################"))<<endl;
	if(template_name == "BDT" || template_name == "BDTttZ" || template_name == "mTW" || template_name == "mTWandBDT" || template_name == "BDTfake" || template_name == "BDT0l") {cout<<FYEL("--- Producing "<<template_name<<" Template Plots ---")<<endl;}
	...
	//Create TGraphAsymmErrors with the error vectors / (x,y) coordinates --> Can superimpose it on plot
	TGraphAsymmErrors* gr_error = new TGraphAsymmErrors(nofbins,x,y,exl,exh,eyl,eyh);
	// gr_error->SetFillStyle(3013);
	gr_error->SetFillStyle(3002);
	gr_error->SetFillColor(1);
	gr_error->Draw("e2 same"); //Superimposes the systematics uncertainties on stack
	...
	if (template_name == "BDT" || template_name == "BDTttZ") {histo_ratio_data->GetXaxis()->SetTitle("BDT output");} //CHANGED
	...
	if(template_name=="mTW") {stack_MC->GetYaxis()->SetTitle("Events/25 GeV");}
	else if(template_name == "BDTttZ") {stack_MC->GetYaxis()->SetTitle("Events/0.2");}
...
*/
	...
	if(isWZ) {extrainfo_data= "0bjet";} //CHANGED
	else if(isttZ) {extrainfo_data= "2bjets";}
	else if(!isWZ && !isttZ) {extrainfo_data= "1bjet";}
...
{
	cout<<endl<<BOLD(FYEL("##################################"))<<endl;
	if(template_name == "BDT" || template_name == "BDTttZ" || template_name == "mTW" || template_name == "mTWandBDT" || template_name == "BDTfake" || template_name == "BDT0l") {cout<<FYEL("--- Producing "<<template_name<<" Template Plots from Combine Output ---")<<endl;}
	else {cout<<FRED("--- ERROR : invalid template_name value !")<<endl;}
	cout<<BOLD(FYEL("##################################"))<<endl<<endl;
	...
	//--- All MC samples but fakes
		for(int isample = 0; isample < sample_list.size(); isample++)
		{
			...
			else //MC
			{
				...
				else {cout<<"ERROR --- DIRECTORY NOT FOUND IN FILE ! SKIP"<<endl; continue;}
   ...
   // axis->SetLineColor(1);
	...
	// else if ( template_name == "BDTttZ" ) axis->SetTitle("BDT output (2 tags)");
   	...
   	if (template_name == "BDT" || template_name == "BDTttZ") axis->SetTitle("BDT output") ;
...
// -- using https://twiki.cern.ch/twiki/pub/CMS/Internal/FigGuidelines //CHANGED
		...
		if(template_name=="mTW") {extrainfo_data= "0bjet";}
		else if(template_name == "BDTttZ") {extrainfo_data= "2bjets";}
		else if(template_name == "BDT") {extrainfo_data= "1bjet";}
...
void theMVAtool::Superpose_With_Without_MEM_Templates(TString template_name, TString channel, bool normalized)
{
	...
	// TLegend* qw = new TLegend(.35,.65,0.75,0.87);
	TLegend* qw = new TLegend(.37,.65,0.77,0.87);
	qw->SetShadowColor(0);
	qw->SetFillColor(0);
	qw->SetLineColor(0);
	// qw->SetLineWidth(3);
	...
	if(!normalized) //Changes which histo should be drawn first
	{
		...
		if(template_name.Contains("BDT") ) h_sum_background_MEM->GetXaxis()->SetTitle("BDT output"); //CHANGED
	...
	else
	{
		...
		if(template_name.Contains("BDT") ) h_signal_MEM->GetXaxis()->SetTitle("BDT output");
	...
	Load_Canvas_Style();
	TCanvas* c1 = new TCanvas("c1","c1", 1000, 800);
	c1->SetTopMargin(0.1); //Leave space for legend --only if there isn't 'Prelim' label
	...
	qw->AddEntry(h_sum_background_noMEM, "Background" , "L");
	qw->AddEntry(h_sum_background_MEM, "Background (with MEM)" , "L");
...
// -- using https://twiki.cern.ch/twiki/pub/CMS/Internal/FigGuidelines //CHANGED
	...
	if(draw_preliminary_label) extraText   += " Preliminary";
	float extraTextFont = 52;  // default is helvetica-italics
	...
	latex.SetTextSize(lumiTextSize*t);
	// latex.DrawLatex(0.7,1-t+lumiTextOffset*t,lumi_13TeV);
	latex.DrawLatex(0.97,1-t+lumiTextOffset*t,lumi_13TeV); //CHANGED
...
//Retrieve histograms, stack, compare, plot
	for(int ivar=0; ivar<template_list.size(); ivar++)
	{
		...
		TGraphAsymmErrors* gr_error = 0;
		gr_error = new TGraphAsymmErrors(nofbins,x,y,exl,exh,eyl,eyh);
		gr_error->SetFillStyle(3005); //Right strips
		gr_error->SetFillColor(2);
		gr_error->Draw("e2 same"); //Superimposes the systematics uncertainties on stack

		qw->AddEntry(gr_error, (systematic+" with MEM").Data(), "f");
...
//------------------
	...
	//Create Data/MC ratio plot (bottom of canvas)
		...
		//--> Create new TGraphAsymmErrors
		TGraphAsymmErrors *gr_ratio_error = new TGraphAsymmErrors(thegraph_tmp->GetN(), theX , theY ,  theErrorX_l, theErrorX_h, theErrorY_l, theErrorY_h);
		gr_ratio_error->SetFillStyle(3005);
		gr_ratio_error->SetFillColor(2);
		gr_ratio_error->Draw("e2 same"); //Syst. error for Data/MC ; drawn on canvas2 (Data/MC ratio)
		...
		//--> Create new TGraphAsymmErrors
		TGraphAsymmErrors *gr_ratio_error_noMEM = new TGraphAsymmErrors(thegraph_tmp_noMEM->GetN(), theX_noMEM , theY_noMEM ,  theErrorX_l_noMEM, theErrorX_h_noMEM, theErrorY_l_noMEM, theErrorY_h_noMEM);
		gr_ratio_error_noMEM->SetFillStyle(3004);
		gr_ratio_error_noMEM->SetFillColor(1);
		gr_ratio_error_noMEM->Draw("e2 same"); //Syst. error for Data/MC ; drawn on canvas2 (Data/MC ratio)
 ...
 */
void theMVAtool::Superpose_Shapes_Fakes_Signal(TString template_name, TString channel, bool normalized, bool draw_templates, bool separate_fakes)
{
	if(channel != "allchan" && channel != "uuu" && channel != "uue" && channel != "eeu" && channel != "eee" )
	{
		cout<<"Wrong channel  Abort"<<endl; return;
	...
	for(int ivar=0; ivar<total_var_list.size(); ivar++)
	{
		...
		//--- All MC samples but fakes
			for(int isample = 0; isample < sample_list.size(); isample++)
			{
				...
				else if(sample_list[isample].Contains("Fakes") ) //fakes
				{
					...
					if(separate_fakes)
					{
						if(sample_list[isample].Contains("Muon"))
						{
							if(h_fakes == 0) {h_fakes = (TH1F*) h_tmp->Clone();}
							else {h_fakes->Add(h_tmp);}
						}
						else if(sample_list[isample].Contains("Electron"))
						{
		...
		if(normalized)
		{
			...
			if(separate_fakes) h_fakes2->Scale(1./h_fakes2->Integral() );
		}
		...
		if(separate_fakes)
		{
			h_fakes2->SetLineColor(kAzure-4);
		...
		else if(varname_tmp == "MEMvar_1")             {       title_tmp = "MEM log w_{tZq}"; }
		...
		{
			if(separate_fakes && h_fakes2->GetMaximum() > h_fakes->GetMaximum()) h_signal->SetMaximum(h_fakes2->GetMaximum()*1.2);
			else h_signal->SetMaximum(h_fakes->GetMaximum()*1.2);
		}
		...
		if(draw_templates)
		{
			...
			if(separate_fakes) h_fakes2->Draw("same hist E");
		}
		else
		{
			h_signal->Draw("hist");
			...
			if(separate_fakes) h_fakes2->Draw("same hist");
		}

		qw->AddEntry(h_signal, "tZq" , "L");
		if(separate_fakes)
		{
			qw->AddEntry(h_fakes, "Non-prompt Muons" , "L");

			qw->AddEntry(h_fakes2, "Non-prompt Electrons" , "L");
		}
		else qw->AddEntry(h_fakes, "Non-prompt" , "L");
	...
	//------------------
		...
		if(channel.Contains("all") && !draw_templates) output_plot_name+= "allchans/";
		...
		delete qw;
		delete c1;
		if(separate_fakes) delete h_fakes2;
...
int theMVAtool::Draw_Control_Plots_ForPaper()
{
	cout<<endl<<BOLD(FYEL("##################################"))<<endl;
	cout<<FYEL("--- Draw Control Plots ---")<<endl;
	cout<<BOLD(FYEL("##################################"))<<endl<<endl;
	...
	cout<<endl<<FMAG("--- Will use file ./"<<input_file_name<<" to extract signal strength (rescale signal) !")<<endl;


	//--------------------------
	// DIVIDE CANVAS IN 6 PADS
	//--------------------------
	...
	vector<TH1F*> v_hdata(6);
	vector<THStack*> v_stack(6);
	vector<TH1F*> v_histo_total_MC(6);
	vector<vector<TH1F*> > v_vector_MC_histo(6); //Store separately the histos for each MC sample --> stack them after loops
	vector<TH1F*> v_histo_ratio_data(6);

	vector<TGraphAsymmErrors*> v_gr_error_tmp(6);
	...
	for(int iplot=0; iplot<6; iplot++)
	{
		...
		if(iplot==0)
		{
			...
			if(right_margin) pad->SetRightMargin(margin); //FIXME
		}
		else if(iplot==1)
		{
			TPad* pad = (TPad*) c1->GetPrimitive(primitive_name);
		...
		if(iplot==0)
		{
			input_file_name = "outputs/PostfitInputVars_tZq.root";
		...
		{
			input_file_name = "outputs/PostfitInputVars_ttZ.root";
			cout<<FMAG("--- Will use file ./"<<input_file_name<<" to get postfit plots !")<<endl<<endl;
			f = TFile::Open( input_file_name );
			if(f == 0) {cout<<endl<<BOLD(FRED("--- File not found ! Exit !"))<<endl<<endl; return 0;}
		}
		...
		if(iplot<3)	{gPad->SetBottomMargin(0.25);}
		else	{gPad->SetBottomMargin(0.27);}
		...
		//Only call this 'random' histogram here in order to get the binning used for the current variable --> can initialize the error vectors !
		if (!f->GetDirectory((total_var_list[ivar] + "_uuu_postfit").Data()) )
		{
			cout<<total_var_list[ivar].Data()<<"_uuu_postfit not found !"<<endl;
			return 0;
		}
		...
		TString histo_name = "";
		histo_name =  total_var_list[ivar] + "_uuu_postfit/tZqmcNLO"; //For postfit file only !
		...
		for(int ichan=0; ichan<channel_list.size(); ichan++)
		{
			for(int isample = 0; isample < sample_list.size(); isample++)
			{
				...
				histo_name = total_var_list[ivar] + "_" + channel_list[ichan] + "_postfit/";
				if(!f->GetDirectory(histo_name)) {cout<<BOLD(FRED("Error : directory"<<histo_name<<" not found !"))<<endl; continue;}
				...
				else //MC
				{
					...
					if(sample_list[isample].Contains("tZq")) {h_tmp->Scale(sig_strength);} //Need to rescale signal manually, not done by CombineHarvester !!
		...
		{
			//Data on top of legend
			if(v_hdata[iplot] != 0) {qw->AddEntry(v_hdata[iplot], "  Data" , "ep");}
			else {cout<<__LINE__<<BOLD(FRED(" : h_data is null"))<<endl;}
		...
		if(right_margin) v_tpad_ratio[iplot]->SetRightMargin(margin); //FIXME
		...
		for(int ibin=1; ibin<v_histo_ratio_data[iplot]->GetNbinsX()+1; ibin++)
		{
			...
			double bin_error = pow(pow(v_histo_total_MC[iplot]->GetBinError(ibin), 2) + pow(v_histo_ratio_data[iplot]->GetBinError(ibin), 2) + pow(bin_error_mu, 2), 0.5);
	...
	TString outputname = "plots/paper_plots/postfit_input_tZq_ttZ_all";
	outputname+= this->format;
...
int theMVAtool::Draw_Control_Plots_ForPaper_WZ()
{
	cout<<endl<<BOLD(FYEL("##################################"))<<endl;
	cout<<FYEL("--- Draw Control Plots Paper WZ ---")<<endl;
	cout<<BOLD(FYEL("##################################"))<<endl<<endl;
	...
	// TCanvas* c1 = new TCanvas("c1","c1", 1350, 400);
	// c1->Divide(3, 2, 0.01, 0); //colums, rows
	// c1->Divide(3,2); //colums, rows
	c1->Divide(3,1, 0.01, 0.00001, 0);
	...
	input_file_name = "outputs/PostfitInputVars_WZ.root";
	cout<<FMAG("--- Will use file ./"<<input_file_name<<" to get postfit plots !")<<endl;
	f = TFile::Open( input_file_name );
	if(f == 0) {cout<<endl<<BOLD(FRED("--- File not found ! Exit !"))<<endl<<endl; return 0;}
	...
	for(int iplot=0; iplot<3; iplot++)
	{
		...
		{
			TPad* pad = (TPad*) c1->GetPrimitive(primitive_name);
			pad->SetPad(0, 0., 0.33, 1); //xlow, ylow, xup, yup
			if(right_margin) pad->SetRightMargin(margin); //FIXME
		}
		else if(iplot==1)
		{
		...
		// if(iplot==1) v_tpad_ratio[iplot]->SetRightMargin(margin); //FIXME
		if(right_margin) v_tpad_ratio[iplot]->SetRightMargin(margin); //FIXME
	...
	TString region_name = "WZ_CR";


	mkdir("plots/paper_plots",0777);

	//Iximumge name
	TString outputname = "plots/paper_plots/postfit_input_WZ_all";
	outputname+= this->format;
...
int theMVAtool::Postfit_Templates_Paper()
{
	cout<<endl<<BOLD(FYEL("##################################"))<<endl;
	...
	for(int iplot=0; iplot<3; iplot++)
	{
		...
		if(iplot==0)
		{
			...
			if(right_margin) pad->SetRightMargin(margin);
		}
		else if(iplot==1)
		{
			TPad* pad = (TPad*) c1->GetPrimitive(primitive_name);
...
//--- Provide histograms for Denis Gele (IPHC), for a study
void theMVAtool::Histograms_For_Denis()
{
	TString path_data = "./outputs/default/Combine_Input.root"; //use prefit histo
	TFile* file_data = TFile::Open(path_data);

	TString path = "./outputs/default/mlfit.root"; //postfit histos
	TFile* f = TFile::Open(path);

	TString output_path = "./outputs/Histograms_tZq.root"; //output
	TFile* f_output = TFile::Open( output_path, "RECREATE" );

	TH1F *h_tmp = 0, *h_sum_tmp = 0, *h_sum_bkg = 0, *h_sum_fake = 0;

	// TString template_name = "BDT";
	// TString template_name = "BDTttZ";
	TString template_name = "mTW";
	...
	for(int isample=0; isample<sample_list.size(); isample++)
	{
		...
		for(int ichan=0; ichan<channel_list.size(); ichan++)
		{
			// cout<<sample_list[isample]<<" -- "<<channel_list[ichan]<<endl;

			TString dir_name = "";
			TString hname = "";
			if(sample_list[isample] == "Data") {hname = template_name + "_" + channel_list[ichan] + "__data_obs";}
			else
			{
				dir_name = "shapes_fit_s/" + template_name + "_" + channel_list[ichan];
				hname = sample_list[isample];
			}

			if(sample_list[isample] == "Data")
			{
				h_tmp = (TH1F*) file_data->Get(hname)->Clone();

				if(!h_sum_tmp) {h_sum_tmp = (TH1F*) h_tmp->Clone();}
				else {h_sum_tmp->Add(h_tmp);}
			}
			else
			{
				if(sample_list[isample].Contains("Muon") && channel_list[ichan] == "eee") {continue;}
				else if(sample_list[isample].Contains("Electron") && channel_list[ichan] == "uuu") {continue;}

				//Only call this 'random' histogram here in order to get the binning used for the current variable --> can initialize the error vectors !
				if(f->GetDirectory(dir_name))
				{
					h_tmp = (TH1F*) f->Get(dir_name + "/" + hname)->Clone();
				}
				else {cout<<__LINE__<<FRED(" : "<<dir_name<<" not found ! Can not initialize error vectors ! (This is hard-coded)")<<endl;}

				if(!sample_list[isample].Contains("Fake"))
				{
					if(!h_sum_tmp) {h_sum_tmp = (TH1F*) h_tmp->Clone();}
					else {h_sum_tmp->Add(h_tmp);}
				}

				if(sample_list[isample] != "Data" && !sample_list[isample].Contains("tZq") )
				{
					if(!h_sum_bkg) {h_sum_bkg = (TH1F*) h_tmp->Clone();}
					else {h_sum_bkg->Add(h_tmp);}
				}

				if(sample_list[isample].Contains("Fake"))
				{
					if(!h_sum_fake) {h_sum_fake = (TH1F*) h_tmp->Clone();}
					else {h_sum_fake->Add(h_tmp);}
				}
			}

			nevents+= h_tmp->Integral();


			cout<<sample_list[isample]<<" -- "<<channel_list[ichan]<<" -- "<<nevents<<" events : done"<<endl;
		}

		if(!sample_list[isample].Contains("Fake"))
		{
			f_output->cd();
			h_sum_tmp->Write(sample_list[isample]);
		}
	}

	f_output->cd();
	h_sum_bkg->Write("sum_backgrounds");
	h_sum_fake->Write("Fakes");
